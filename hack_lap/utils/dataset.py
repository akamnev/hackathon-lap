...
def get_gcn_dataset(filename, dev_frac=0.1, dev_reverse=False, positive_multiplier=1, seed=None):
    random.seed(seed)
...
def get_eval_dataset(filename):
    with open(filename, 'rb') as fp:
        data = pickle.load(fp)
    return _gcn_eval_dataset(data)
