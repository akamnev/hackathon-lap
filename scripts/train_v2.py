...
def main(
    ...
    model_name = os.path.join(DIR_MODEL, f'{exp_name}-Nepoch-{start_epoch}.pt')
    ...
    scheduler = torch.optim.lr_scheduler.ExponentialLR(opt, gamma=lrg)
    fn_loss = torch.nn.BCEWithLogitsLoss()
    fn_proba = torch.nn.Sigmoid()

    for n_epoch in trange(start_epoch, start_epoch + num_epoch):
        ...
        if n_epoch % 5 == 0:
            ...
            bce_loss, (precision_0, recall_0, f1_0), (precision_1, recall_1, f1_1) \
                = calculate_metrics_one_vs_rest(model, loader_dev, device, repeat=repeat)
    ...
    output_filename = f'{exp_name}-Nepoch-{start_epoch + num_epoch}'
    torch.save(
        model.state_dict(),
    ...
    cfg = {
        ...
        'num_epoch': start_epoch + num_epoch,
        'repeat': repeat,
        'lr': lr,
        'wd': wd,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lrg', dest='lrg', default=0.97, type=float)
