"""скрипт поиска параметров обучения"""
import os
import argparse
import json
import pickle
...
DIR_LOG = os.path.join(DIR_DATA, 'search_log')
DIR_MODEL = os.path.join(DIR_DATA, 'search_model')
...
def main(
    ...
    model_name = 'mdl-' + exp_name + '.pt'
    exp_name += f'-p_mult: {positive_multiplier} decay: {wd} kld: {weight_kld} entropy: {weight_entropy}'
    ...
    model.load_state_dict(torch.load(
        os.path.join(DIR_MODEL, model_name),
        map_location='cpu'
    ))
    model.train()
    model.to(device)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_entropy', dest='weight_entropy', default=0.1, type=float)
    ...
    parser.add_argument('--seed', dest='seed', default=20220216, type=int)

    params = parser.parse_args()

    positive_multiplier = [5, 10, 15, 20, 25, 30]
    wd = [0.0, 0.05, 0.1, 0.2, 0.3]
    weight_kld = [1e-7, 1e-6, 5e-6, 1e-5]
    weight_entropy = [0.0, 0.1, 0.2, 0.5]
    pp = list(product(positive_multiplier, wd, weight_kld, weight_entropy))
    random.shuffle(pp)

    for positive_multiplier, wd, weight_kld, weight_entropy in pp:
